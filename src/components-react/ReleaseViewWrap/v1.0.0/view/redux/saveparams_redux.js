/** 存储所有数据的redux */
const SAVEPRAMAS = 'SAVEPRAMAS';

//初始化保存的数据
const itemTmplPublishVo = {
    "itemTmplPublishVo":{  //每一个小的vo都要先转成json
      "id": null,//商品id
      "shopId": null,//店铺id
      "sellerId": null,//卖家id
      ...
      "itemPerfectVo":{
          ...
          "placeDeliveryId":null,//["101704"],//供货信息页发货地(有则必填)
          "preSkuPriceVoList": [{ //(必有，是否必填可配置) 
            ...
            "attributes":null,//[{'aid': 2641, 'aName': "型号千3", 'vid': 6547, 'vName': "M"}]//销售属性[{"aid":123,"vid":456}](已有)
            "isPrice":0,
          }]
      },
      "extendFields": [
...
export default function (state = itemTmplPublishVo, action = {}) {
    switch (action.type) {
        case SAVEPRAMAS:
            return {
                ...state,
...
export function saveParamsAction(data) {
    return {
        type: SAVEPRAMAS,
        data: data
    }
